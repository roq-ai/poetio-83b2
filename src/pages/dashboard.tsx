...
import { useRoqPlatformUserProfiles, usePoetryCount, useContentCreatorCount, useVideoCount } from 'lib/roq';

import { ContentCreatorListPage as ShowCaseList } from 'pages/content-creators';

function DashboardPage() {
  const { session } = useSession();

  const poetryStat = usePoetryCount();

  const contentCreatorStat = useContentCreatorCount();

  const videoStat = useVideoCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Poetries',
      result: poetryStat?.data,
      isLoading: poetryStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Content Creators',
      result: contentCreatorStat?.data,
      isLoading: contentCreatorStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Videos',
      result: videoStat?.data,
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Content Creators
                </Text>
                <NextLink href={`content-creators`} passHref legacyBehavior>
                  <Button
